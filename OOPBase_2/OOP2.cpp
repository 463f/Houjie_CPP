...
template <typename T>
using Lst =
    list<T, allocator<T>>;  // Note: make sure your environment support C++2.0

void test_template_template_parameters_1() {
  cout << "test_template_template_parameters_1()" << endl;

  //!	XCls<string, list> mylist;
  //[Error] expected a template of type 'template<class T> class Container', got
...
void test_template_template_parameters_2() {
  ...
  XCls<string, shared_ptr>
      p1;  // Note: make sure your environment support C++2.0
  XCls<double, auto_ptr> p4;
  //!	XCls<double, unique_ptr> p2;  	//unique_ptr has two template parameters
  //!	XCls<int, weak_ptr> p3;			//[Error] no matching function for call
  //!to 'std::weak_ptr<int>::weak_ptr(int*)'
}
}  // namespace jj03
//-------------------------------------------------------------
#include <iostream>
...
void test_object_model() { cout << "test_object_model()" << endl; }
}  // namespace jj04
//-------------------------------------------------------------
#include <iostream>
namespace jj05 {
